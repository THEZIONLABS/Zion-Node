...
func (d *Daemon) processHubCommand(ctx context.Context, cmd *types.HubCommand) {
	...
	case "chat_now":
		// Nudge agent to join danmaku chat — hub provides the full message via params.
		message, _ := cmd.Params["message"].(string)
		if message == "" {
			log.Warn("chat_now command missing message param, skipping")
			return
		}
		if err := d.agentManager.SendPrompt(cmdCtx, cmd.AgentID, message); err != nil {
...
func (d *Daemon) buildAgentEnv(params map[string]interface{}) map[string]string {
	...
	// Platform context → soul.md and heartbeat.md injection for Gateway-created agents
	if v, ok := params["soul_context"].(string); ok && v != "" {
		env["ZION_SOUL_CONTEXT"] = v
	}
	if v, ok := params["heartbeat_context"].(string); ok && v != "" {
		env["ZION_HEARTBEAT_CONTEXT"] = v
	}
	if v, ok := params["bootstrap_context"].(string); ok && v != "" {
		env["ZION_BOOTSTRAP_CONTEXT"] = v
	}
